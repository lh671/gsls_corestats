...
```{r}
rivers<-read.csv(file.choose(), header=T)
...
o The calculated t-value is 3.8433 (we need this for reporting)
o There are 66 degrees of freedom (we need this for reporting)
o The p-value is 0.0002754.
...
A Student’s t-test indicated that the mean body length of male guppies in the Guanapo river (18.29mm) differs significantly from the mean body length of male guppies in the Aripo river(20.33mm) (t=3.8433, df=66, p=0.0003).
...
The 1st line gives the name of the test and the 2nd line reminds you what the dataset was called, and what variables were used
The 3rd line contains the two key outputs from the test:
o The calculated W-value is 841 (we’ll use this in reporting)
o The p-value is 0.0006464.
...
ANSWER: A Mann-Whitney test indicated that the median body length of male guppies in the Guanapo river (18.8mm) differs significantly from the median body length of male guppies in the Aripo river(20.1mm) (W=841, p=0.0006).
...
```{r}
cortisol <- read.csv(file.choose(),header=T)
...
A two-tailed, paired t-test indicated that the cortisol level in adult females differed significantly between the morning (313.5nmol/l) and the evening (197.4nmol/l) (t=-5.1833, df=19, p=5.3x10-5).
...
A two-tailed, Wilcoxon signed-rank test indicated that the median cortisol level in adult females differed significantly between the morning (320.5nmol/l) and the evening (188.9nmol/l) (V=197, p=0.00017).